...
class MLPValidator(Validator):
    ...
    def validate(self, coords, values):
        model_results = self.model(
            {'coords': self.dynamics.coord_to_input(coords.to(self.dynamics.device))})
        inputs = torch.cat(
            (coords[..., 1:].to(self.dynamics.device), values[:, None].to(self.dynamics.device)), dim=-1)
...
def scenario_optimization(model,  dynamics, tMin, tMax, dt, set_type, control_type, scenario_batch_size, sample_batch_size, sample_generator, sample_validator, violation_validator, max_scenarios=None, max_samples=None, max_violations=None, tStart_generator=None):
    policy = model
    rem = ((tMax-tMin) / dt) % 1
    e_tol = 1e-12
    ...
    while True:
        ...
        while num_collected_scenarios < scenario_batch_size:
            ...
            with torch.no_grad():
                batch_sample_model_results = model(
                    {'coords': dynamics.coord_to_input(batch_sample_coords.to(dynamics.device))})
        ...
        for k in tqdm(range(int((tMax-tMin)/dt)), desc='Trajectory Propagation', position=pbar_pos, leave=False):
            ...
            traj_policy_results = policy(
                {'coords': dynamics.coord_to_input(traj_coords.to(dynamics.device))})
            ...
            ctrl_trajs[:, k] = dynamics.optimal_control(
                traj_coords[:, 1:].to(dynamics.device), traj_dvs[..., 1:].to(dynamics.device))
            dstb_trajs[:, k] = dynamics.optimal_disturbance(
                traj_coords[:, 1:].to(dynamics.device), traj_dvs[..., 1:].to(dynamics.device))
            # ham_trajs[:, k] = dynamics.hamiltonian(
            #     traj_coords[:, 1:].cuda(0), traj_dvs[..., 1:].cuda(0)) # No need to compute this
            ...
            if tStart_generator is not None:  # freeze states whose start time has not been reached yet
                ...
                # state_trajs[is_unfrozen, k+1] = dynamics.equivalent_wrapped_state(state_trajs[is_unfrozen, k].cuda(0) + dt*dynamics.dsdt(
                #     state_trajs[is_unfrozen, k].cuda(0), ctrl_trajs[is_unfrozen, k].cuda(0), dstb_trajs[is_unfrozen, k].cuda(0), torch.ones((scenario_batch_size, 1)).cuda(0))).cpu()
                state_trajs[is_unfrozen, k+1] = dynamics.equivalent_wrapped_state(state_trajs[is_unfrozen, k].to(dynamics.device) + dt*dynamics.dsdt(
                    state_trajs[is_unfrozen, k].to(dynamics.device), ctrl_trajs[is_unfrozen, k].to(dynamics.device), dstb_trajs[is_unfrozen, k].to(dynamics.device)).to(dynamics.device)).cpu()
            else:
                next_state_ = dynamics.equivalent_wrapped_state(state_trajs[:, k].to(dynamics.device
                ) + dt*dynamics.dsdt(state_trajs[:, k].to(dynamics.device), ctrl_trajs[:, k].to(dynamics.device), dstb_trajs[:, k].to(dynamics.device)))
        ...
        if set_type == 'BRT':
            batch_scenario_costs = dynamics.cost_fn(state_trajs.to(dynamics.device))
        elif set_type == 'BRS':
            if control_type == 'init_ttr':  # is this correct for init_ttr?
                batch_scenario_costs = dynamics.boundary_fn(
                    state_trajs.to(dynamics.device))[:, (init_traj_times - tMin) / dt]
            elif control_type == 'value':
                batch_scenario_costs = dynamics.boundary_fn(
                    state_trajs.to(dynamics.device))[:, -1]
            else:
    ...
    with torch.no_grad():
        ...
        while len(states) < num_samples:
            ...
            batch_model_results = model(
                {'coords': dynamics.coord_to_input(batch_coords.to(dynamics.device))})
        ...
        valids = target_validator.validate(coords.to(dynamics.device), values.to(dynamics.device))
